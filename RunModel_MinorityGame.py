...
def update_scores_and_strategy():
    global turtles, strategies_scores, minority, history, current_strategy
    increment_scores()
...
for number_of_agents in (11, 25, 101, 1001):
    print('number_of_agents:', number_of_agents)
    for strategies_per_agent in (2, 6, 3, 4, 8, 16):
        ...
        for memory in range(2, memory_max + 1):
            ...
            for simulation in range(max_simulation):
                ...
                # result.to_csv('Result/ReplicateNetLogo_Result_N' + str(number_of_agents) + '_s_' + str(strategies_per_agent) + '.csv', index=False)
        result.to_csv('Result/ReplicateNetLogo_Result_N_' + str(number_of_agents) + '_s_' + str(strategies_per_agent) + '.csv', index=False)